...
plt.switch_backend("Agg")  # non‑interactive backend for Streamlit
...
def place_label(ax, placed, x, y, text, color="blue"):
    """Place a label, nudging if it would overlap an existing label."""
    dx = dy = 0.0
    for xx, yy in placed:
...
def html_frame(body: str, height: int = 450):
    components.html(f"<html><body>{body}</body></html>",
...
def wrap_table(css: str, inner: str) -> str:
...
def t_tab():
    ...
    with c2:
        alpha = st.number_input("α", 0.05, 0.01, min_value=0.0001,
                                max_value=0.5, key="t_alpha")
...
def t_table(df, a, tail, key):
    ...
    def crit(d,m,al): return stats.t.ppf(1-al/(1 if m=="one" else 2), d)
    ...
    html=wrap_table(css,f"<tr><th>df</th>{header}</tr>{body}")
    ...
    if step>=0:
        for i in range(len(heads)+1):
            html=style_cell(html,f"df_{df}_{i}")
    if step>=1:
        for d in dfs: html=style_cell(html,f"df_{d}_{col_idx}")
    if step>=2:
        html=style_cell(html,f"df_{df}_{col_idx}","blue",3)
    if step>=3 and tail=="one‑tailed" and abs(a-.05)<1e-12:
        alt=heads.index(("two",.10))+1
        for d in dfs: html=style_cell(html,f"df_{d}_{alt}")
        html=style_cell(html,f"df_{df}_{alt}","blue",3)
...
def z_tab():
    ...
    c1,c2=st.columns(2)
    with c1:
        z_val=st.number_input("z statistic",1.64,key="z_val")
    with c2:
        alpha=st.number_input("α",0.05,0.01,min_value=0.0001,
                              max_value=0.5,key="z_alpha")
...
def plot_z(z,a,tail):
    ...
    if tail=="one‑tailed":
        ...
        ax.fill_between(x[x>=crit],y[x>=crit],color="red",alpha=.3,
                        label="Reject H₀")
        ax.axvline(crit,'g','--')
...
def z_table(z_in,key):
    ...
    css=("table{border-collapse:collapse}"
         ...
         "text-align:center;font-size:.9rem}")
    html=wrap_table(css,f"<tr><th>z.x</th>{header}</tr>{body}")

    if step>=0:
        for c in cols: html=style_cell(html,f"z_{row:.1f}_{c:.2f}")
        html=style_cell(html,f"z_{row:.1f}_0")
    if step>=1:
        for r in subset: html=style_cell(html,f"z_{r:.1f}_{col:.2f}")
    if step>=2:
        html=style_cell(html,f"z_{row:.1f}_{col:.2f}","blue",3)
...
def f_tab():
    ...
    c1,c2=st.columns(2)
    with c1:
        f_val=st.number_input("F statistic",4.32,key="f_val")
        df1  =st.number_input("df₁ (numerator)",1,step=1,value=5,key="f_df1")
    with c2:
        df2  =st.number_input("df₂ (denominator)",1,step=1,value=20,key="f_df2")
        alpha=st.number_input("α",0.05,0.01,min_value=0.0001,
                              max_value=0.5,key="f_alpha")
...
def f_table(df1,df2,a,key):
    ...
    html=wrap_table(css,f"<tr><th>df₁＼df₂</th>{header}</tr>{body}")
    ...
    if step>=2:
        html=style_cell(html,f"f_{df1}_{col_idx}","blue",3)
...
def chi_tab():
    ...
    c1,c2=st.columns(2)
    with c1:
        chi_val=st.number_input("χ² statistic",7.88,key="chi_val")
        df     =st.number_input("df",1,step=1,value=3,key="chi_df")
    with c2:
        alpha=st.selectbox("α",[.10,.05,.01,.001],index=1,key="chi_alpha")
...
def chi_table(df,a,key):
    ...
    css=("table{border-collapse:collapse}"
         "td,th{border:1px solid#000;width:80px;height:30px;"
         "text-align:center;font-size:.85rem}")
    html=wrap_table(css,f"<tr><th>df＼α</th>{header}</tr>{body}")
    ...
    if step>=2:
        html=style_cell(html,f"chi_{df}_{col_idx}","blue",3)
...
def u_tab():
    ...
    c1,c2=st.columns(2)
    with c1:
        u_val=st.number_input("U statistic",23,key="u_val")
        n1   =st.number_input("n₁",2,step=1,value=10,key="u_n1")
    with c2:
        n2   =st.number_input("n₂",2,step=1,value=12,key="u_n2")
        alpha=st.number_input("α",0.05,0.01,min_value=0.0001,
                              max_value=0.5,key="u_alpha")
...
def u_table(n1,n2,a,tail,key):
    ...
    html=wrap_table(css,f"<tr><th>n₁＼n₂</th>{header}</tr>{body}")
    ...
    if step>=2:
        html=style_cell(html,f"u_{n1}_{col_idx}","blue",3)
...
def w_tab():
    ...
    c1,c2=st.columns(2)
    with c1:
        t_val=st.number_input("T statistic",15,key="w_val")
        n    =st.number_input("N (non‑zero diffs)",5,step=1,value=12,key="w_n")
    with c2:
        alpha=st.number_input("α",0.05,0.01,min_value=0.0001,
                              max_value=0.5,key="w_alpha")
...
def w_table(n,a,tail,key):
    ...
    css=("table{border-collapse:collapse}"
         "td,th{border:1px solid#000;width:80px;height:30px;"
         "text-align:center;font-size:.8rem}")
    html=wrap_table(css,f"<tr><th>N＼α</th>{header}</tr>{body}")
    ...
    if step>=2:
        html=style_cell(html,f"w_{n}_{col_idx}","blue",3)
...
def binom_tab():
    ...
    c1,c2=st.columns(2)
    with c1:
        n=st.number_input("n (trials)",1,step=1,value=20,key="b_n")
        p=st.number_input("π (null proportion)",.50,.01,min_value=0.01,
                          max_value=.99,key="b_p")
    with c2:
        k=st.number_input("k (successes)",0,step=1,value=12,key="b_k")
        alpha=st.number_input("α (two‑tailed)",0.05,0.01,min_value=0.0001,
                              max_value=0.5,key="b_alpha")
...
def binom_table(k,n,p,key):
    ...
    css=("table{border-collapse:collapse}"
         ...
         "text-align:center;font-size:.8rem}")
    html=wrap_table(css,f"<tr><th>k</th>{header}</tr>{body}")

    if step>=0:
        for i in range(4): html=style_cell(html,f"b_{k}_{i}")
    if step>=1:
        html=style_cell(html,f"b_{k}_1","blue",3)
