...
#
#  Below, we have added the "reason" text for each decision line in the APA 
#  interpretation, but everything else is the same as your prior code.
###############################################################################
...
def build_z_html(z: float, alpha: float, tail: str, step: int) -> str:
    ...
    if step>=0:
        for c_ in Cols: html=style_cell(html,f"z_{row:.1f}_{c_:.2f}")
        html=style_cell(html,f"z_{row:.1f}_0")
    if step>=1:
        for r_ in rows: html=style_cell(html,f"z_{r_:.1f}_{col:.2f}")
    if step>=2:
...
def u_apa(u_val: int, n1: int, n2: int, alpha: float, tail: str):
    ...
    # -- ADDED "REASON" TEXT --
    if reject:
        reason_stats = "because U₍calc₎ was within the rejection region"
        reason_p = "because p < α"
    else:
...
def w_apa(t_val: int, n: int, alpha: float, tail: str):
    ...
    # -- ADDED "REASON" TEXT --
    if reject:
        reason_stats = "because T₍calc₎ was in the rejection region"
        reason_p = "because p < α"
    else:
        reason_stats = "because T₍calc₎ was not in the rejection region"
        reason_p = "because p ≥ α"
...
def binom_apa(k: int, n: int, p: float, alpha: float):
    ...
    # -- ADDED "REASON" TEXT --
    if reject:
        reason_stats = "because k was in the rejection region"
        reason_p = "because p < α"
    else:
