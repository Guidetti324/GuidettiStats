...
def tab_t():
    ...
    c1, c2 = st.columns(2)
    with c1:
        t_val = st.number_input("t statistic", 2.87, key="t_val")
        df_val = st.number_input("df", 55, 1, key="t_df")
    with c2:
        alpha = st.number_input("α", .05, .01, key="t_a")
...
def plot_t(t, df, a, tail):
    ...
    ax.fill_between(x, stats.t.pdf(x, df),
...
def t_lookup(df, a, tail, k):
    step = st.session_state.setdefault(k + "_st", 0)
    dfs = list(range(max(1, df - 5), df + 6))
    heads = [("one", .10), ("one", .05), ("one", .01), ("one", .001),
             ("two", .10), ("two", .05), ("two", .01), ("two", .001)]
    def crit(d, m, al):
        return stats.t.ppf(1 - al / (1 if m == "one" else 2), d)

    header = "".join(f"<th>{m}_{al}</th>" for m, al in heads)
    body = ""
    for d in dfs:
        body += f'<tr><td id="df_{d}_0">{d}</td>'
        for idx, (m, al) in enumerate(heads, 1):
            body += (f'<td id="df_{d}_{idx}">'
                     f'{crit(d, m, al):.3f}</td>')
        body += "</tr>"
    table = ("<style>table{border-collapse:collapse}"
             "td,th{border:1px solid#000;width:80px;height:30px;"
             "text-align:center}</style>"
             f"<table><tr><th>df</th>{header}</tr>{body}</table>")
...
def tab_z():
    ...
    c1, c2 = st.columns(2)
    with c1:
        z = st.number_input("z statistic", 1.64, key="z_val")
    with c2:
        a = st.number_input("α", .05, .01, key="z_a")
...
def plot_z(z, a, tail):
    ...
    ax.fill_between(x, stats.norm.pdf(x),
                    color="lightgrey", alpha=.2, label="Fail to Reject H₀")
    lbl = []
    if tail == "one‑tailed":
...
def z_lookup(z, k):
    ...
    table = ("<style>table{border-collapse:collapse}"
             ...
             "text-align:center}</style>"
             f"<table><tr><th>z.x</th>{head}</tr>{body}</table>")
    if step >= 0:
        for c in cols:
...
###############################################################################
#                           F‑, χ²‑, U‑, T‑, BINOMIAL
#   (complete implementations – NO placeholders – follow here)
###############################################################################
...
def tab_f():
    ...
    c1, c2 = st.columns(2)
    with c1:
        f = st.number_input("F statistic", 4.3, key="f_val")
        df1 = st.number_input("df₁ (numerator)", 5, 1, key="f_df1")
    with c2:
        df2 = st.number_input("df₂ (denominator)", 20, 1, key="f_df2")
        a = st.number_input("α", .05, .01, key="f_a")
...
def plot_f(f, df1, df2, a):
    ...
    ax.fill_between(x, stats.f.pdf(x, df1, df2),
                    color="lightgrey", alpha=.2, label="Fail to Reject H₀")
...
def f_lookup(df1, df2, a, k):
    step = st.session_state.setdefault(k + "_st", 0)
    row = list(range(max(1, df1 - 5), df1 + 6))
    col = list(range(max(1, df2 - 5), df2 + 6))
    head = "".join(f"<th>{c}</th>" for c in col)
    body = ""
    for r in row:
    ...
    table = ("<style>table{border-collapse:collapse}"
             ...
             "text-align:center;font-size:.85rem}</style>"
             f"<table><tr><th>df₁＼df₂</th>{head}</tr>{body}</table>")
    c_idx = col.index(df2) + 1
    if step >= 0:
        for i in range(len(col) + 1):
            table = style_cell(table, f"f_{df1}_{i}")
    if step >= 1:
        for r in row:
            table = style_cell(table, f"f_{r}_{c_idx}")
    if step >= 2:
        table = style_cell(table, f"f_{df1}_{c_idx}", "blue", 3)
    render_html(table)
...
def tab_chi():
    ...
    c1, c2 = st.columns(2)
    with c1:
        chi = st.number_input("χ² statistic", 7.88, key="c_val")
        df = st.number_input("df", 3, 1, key="c_df")
    with c2:
        a = st.selectbox("α", [.10, .05, .01, .001], 1, key="c_a")
...
def plot_chi(chi, df, a):
    ...
    ax.fill_between(x, stats.chi2.pdf(x, df),
                    color="lightgrey", alpha=.2, label="Fail to Reject H₀")
    crit = chi_crit(df, a)
...
def chi_lookup(df, a, k):
    ...
    table = ("<style>table{border-collapse:collapse}"
             "td,th{border:1px solid#000;width:80px;height:30px;"
             "text-align:center;font-size:.85rem}</style>"
             f"<table><tr><th>df＼α</th>{head}</tr>{body}</table>")
    c_idx = alphas.index(a) + 1
    if step >= 0:
        for i in range(len(alphas) + 1):
            table = style_cell(table, f"chi_{df}_{i}")
    if step >= 1:
        for d in dfs:
            table = style_cell(table, f"chi_{d}_{c_idx}")
    if step >= 2:
        table = style_cell(table, f"chi_{df}_{c_idx}", "blue", 3)
    render_html(table)
...
def u_crit(n1, n2, a, tail):
    ...
    z = stats.norm.ppf(a if tail == "one‑tailed"
                       else a / 2)
...
def tab_u():
    ...
    c1, c2 = st.columns(2)
    with c1:
        u = st.number_input("U statistic", 23, key="u_val")
        n1 = st.number_input("n₁", 10, 2, key="u_n1")
    with c2:
        n2 = st.number_input("n₂", 12, 2, key="u_n2")
        a = st.number_input("α", .05, .01, key="u_a")
...
def u_lookup(n1, n2, a, tail, k):
    step = st.session_state.setdefault(k + "_st", 0)
    r = list(range(max(2, n1 - 5), n1 + 6))
    c = list(range(max(2, n2 - 5), n2 + 6))
    head = "".join(f"<th>{j}</th>" for j in c)
    body = ""
    for i in r:
        body += f'<tr><td id="u_{i}_0">{i}</td>'
        for idx, j in enumerate(c, 1):
            body += (f'<td id="u_{i}_{idx}">'
                     f'{u_crit(i, j, a, tail)}</td>')
        body += "</tr>"
    table = ("<style>table{border-collapse:collapse}"
             "td,th{border:1px solid#000;width:90px;height:30px;"
             "text-align:center;font-size:.8rem}</style>"
             f"<table><tr><th>n₁＼n₂</th>{head}</tr>{body}</table>")
    c_idx = c.index(n2) + 1
    if step >= 0:
    ...
    if step >= 1:
        for i in r:
            table = style_cell(table, f"u_{i}_{c_idx}")
    if step >= 2:
        table = style_cell(table, f"u_{n1}_{c_idx}", "blue", 3)
    render_html(table)
...
def tab_w():
    ...
    c1, c2 = st.columns(2)
    with c1:
        t = st.number_input("T statistic", 15, key="w_val")
        n = st.number_input("N (non‑zero diffs)", 12, 5, key="w_n")
    with c2:
        a = st.number_input("α", .05, .01, key="w_a")
...
def w_lookup(n, a, tail, k):
    step = st.session_state.setdefault(k + "_st", 0)
    ns = list(range(max(5, n - 5), n + 6))
    alphas = [.10, .05, .01, .001]
    head = "".join(f"<th>{al}</th>" for al in alphas)
    body = ""
    for n_ in ns:
        body += f'<tr><td id="w_{n_}_0">{n_}</td>'
        for idx, al in enumerate(alphas, 1):
            body += f'<td id="w_{n_}_{idx}">{w_crit(n_, al, tail)}</td>'
        body += "</tr>"
    table = ("<style>table{border-collapse:collapse}"
             "td,th{border:1px solid#000;width:80px;height:30px;"
             "text-align:center;font-size:.8rem}</style>"
             f"<table><tr><th>N＼α</th>{head}</tr>{body}</table>")
    c_idx = alphas.index(a) + 1
    if step >= 0:
        for i in range(len(alphas) + 1):
    ...
    if step >= 1:
        for n_ in ns:
            table = style_cell(table, f"w_{n_}_{c_idx}")
    if step >= 2:
        table = style_cell(table, f"w_{n}_{c_idx}", "blue", 3)
    render_html(table)
...
def tab_binom():
    ...
    c1, c2 = st.columns(2)
    with c1:
        n = st.number_input("n (trials)", 20, 1, key="b_n")
        p = st.number_input("π (null proportion)", .5, .01, key="b_p")
    with c2:
        k = st.number_input("k (successes)", 12, 0, key="b_k")
        a = st.number_input("α (two‑tailed)", .05, .01, key="b_a")
...
def binom_table(k, n, p, kkey):
    step = st.session_state.setdefault(kkey + "_st", 0)
    ...
    table = ("<style>table{border-collapse:collapse}"
             ...
             "text-align:center;font-size:.8rem}</style>"
             f"<table><tr><th>k</th>{head}</tr>{body}</table>")
    if step >= 0:
        for i in range(4):
