...
plt.switch_backend("Agg")  # always use a non‑interactive backend
...
def place_label(ax, placed, x, y, txt, color="blue"):
    """Add a label without overlapping previous ones."""
    dx = dy = 0.0
    for xx, yy in placed:
...
def next_step(key: str):
    if st.button("Next Step", key=f"{key}_btn"):
        st.session_state[key] += 1

def wrap_table(css: str, rows: str) -> str:
    return f"<style>{css}</style><table>{rows}</table>"
...
def tab_t():
    ...
    with c1:
        ...
        df    = st.number_input("df",           min_value=1, value=55,
                                step=1, key="t_df")
    with c2:
...
def t_table(df, a, tail, key):
    step = st.session_state.setdefault(f"{key}_step", -1)
    ...
    html=wrap_table(css, f"<tr><th>df</th>{header}</tr>{rows}")
    ...
    next_step(f"{key}_step")

###############################################################################
#                               2 • z‑Distribution
###############################################################################

# (Full code identical to working version from the previous message)
# ─── omitted here for brevity, but remains unchanged ───
...
def tab_chi():
    ...
    with c1:
        ...
        df      = st.number_input("df", min_value=1, value=3,
                                  step=1, key="chi_df")
    with c2:
        alpha = st.selectbox("α", [.10,.05,.01,.001], index=1, key="chi_a")
...
###############################################################################
#                     5 • Mann‑Whitney U (U‑Distribution)
###############################################################################
...
def binom_table(k,n,p,key):
    ...
    css=("table{border-collapse:collapse}"
         ...
         "text-align:center;font-size:.8rem}")
    html=wrap_table(css,f"<tr><th>k</th>{header}</tr>{rows}")
