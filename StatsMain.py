...
def plot_t(t_calc, df, input_alpha, tail):
    ...
    try:
        if tail.startswith("one"):
            ...
            ax.axvline(plot_crit_val_two_sided, color="green", ls="--"); ax.axvline(-plot_crit_val_two_sided, color="green", ls="--")
    ...
    except Exception as e: st.warning(f"Could not draw all plot elements: {e}")
...
def z_apa(z_val: float, input_alpha: float, tail: str):
    ...
    if tail.startswith("one"):
        if z_val >=0: expl_z.append(f"For a **one-tailed** (right tail) test, $p = 1 - P(Z \\le {z_val:.2f}) \\approx {1-cdf_z if not np.isnan(cdf_z) else np.nan:.4f}$.")
...
def build_f_table(df1: int, df2: int, input_alpha: float) -> str:
    ...
    for r_f_row in rs:
        ...
        for i,c_val in enumerate(cs,start=1):
            v=np.nan; 
            try: v=stats.f.ppf(1-input_alpha,r_f_row,c_val)
            except Exception: pass
            rc+=f'<td id="f_{r_f_row}_{i}">{v:.2f if not np.isnan(v) else "N/A"}</td>'
        bd+=f"<tr>{rc}</tr>"
...
def f_apa(f_val: float, df1: int, df2: int, input_alpha: float): # Original APA structure
    ...
    rs_f=("because F₍calc₎ exceeded F₍crit₎" if rj else "because F₍calc₎ did not exceed F₍crit₎") 
    rp_f=("because p < α" if rj else "because p ≥ α")
...
def tab_f():
    ...
    with c1: fv=st.number_input("F statistic",value=4.32,step=0.01,key="f_v_w_tab3");d1=st.number_input("df1 (numerator)",min_value=1,value=5,step=1,key="f_d1_w_tab3")
    with c2: d2=st.number_input("df2 (denominator)",min_value=1,value=20,step=1,key="f_d2_w_tab3");ai=st.number_input("Your $\\alpha$ (alpha level)",value=0.05,step=0.001,min_value=0.0001,max_value=0.99,format="%.4f",key="f_ai_w_tab3")
...
def build_chi_table(df: int, alpha_for_highlight: float) -> str:
    ...
    for r_chi_row in rs_c:
        ...
        for i_chi_cell,ac_chi_cell in enumerate(std_as,start=1):
            ...
            rc_c+=f'<td id="chi_{r_chi_row}_{i_chi_cell}">{vc:.2f if not np.isnan(vc) else "N/A"}</td>'
        bd_c+=f"<tr>{rc_c}</tr>"
...
def chi_apa(chi_val: float, df: int, input_alpha: float): # Original APA structure
    ...
    dec="rejected" if rj else "failed to reject"; rs_c="because χ²₍calc₎ exceeded χ²₍crit₎" if rj else "because χ²₍calc₎ did not exceed χ²₍crit₎"; rp_c="because p < α" if rj else "because p ≥ α" 
...
def tab_chi():
    ...
    with c1: cv_in=st.number_input("χ² statistic",value=7.88,step=0.01,key="c_v_w_tab4");dfi_in=st.number_input("df ",min_value=1,value=3,step=1,key="c_df_w_tab4")
    with c2:
        sa_c_options=[0.10,0.05,0.025,0.01,0.005,0.001]; afth_in=st.selectbox("Table's $\\alpha$ for highlight",sa_c_options,index=1,key="c_as_w_tab4")
        aia_in=st.number_input("Your test's $\\alpha$",value=0.05,step=0.001,min_value=0.0001,max_value=0.99,format="%.4f",key="c_aia_w_tab4")
...
def tab_u():
    ...
    if st.session_state.u_show_results:
        if int(n1v_in)<1 or int(n2v_in)<1:st.error("n1 and n2 must be >= 1.")
            ...
            with ce_u_main:
                try: u_apa(int(u_v_in),int(n1v_in),int(n2v_in),float(aiv_in),tiv_in)
